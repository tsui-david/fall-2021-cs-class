    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.key == pygame.K_LEFT:
              ...
              person_1_y += step_size
            elif event.key == pygame.K_SPACE and not has_jumped:
              has_jumped = True

    if not user_died:
      if has_jumped and cur_jump_count < max_jump_height:
        person_1_y -= jump_speed
        cur_jump_count += jump_speed
        print(cur_jump_count, max_jump_height)
      if has_jumped and cur_jump_count >= max_jump_height:
        person_1_y += (jump_speed * .8)
        cur_jump_count += (jump_speed * .8) 

      if has_jumped and cur_jump_count >= max_jump_height * 2:
        person_1_y = 350
        cur_jump_count = 0
        has_jumped = False
